        ...
        try:
            ...
            for person_dict in list_1:
                if person_dict.get('is_closed') == False:
                    ...
                    print(first_name)
                    add_VK_Partners(first_name, last_name, vk_id,
                                    vk_link)  # вызов функции для занесения параметров в БД
    ...
    for event in botLongpoll.listen():
            ...
            if event.obj.message['text'] != '':
                if bot.get_user_id(event) != None:
                    ...
                    if active_user == '':
                        ...
                        msg_id = bot.write_msg(active_user,
                        ...
                        # если ни одного подходящего партнера не найдено, то вывести сообщение с кнопкой "Закрыть"
                            ...
                            bot.write_msg(active_user, 'Пользователей с указанными параметрами не найдено.', 'exit',
                                          True, msg_id)
                    ...
                    if active_user != '':
                        ...
                        if active_user != bot.get_user_id(event):
                            ...
                            bot.write_msg(bot.get_user_id(event),
        ...
        # если событие явлется событием чата
            ...
            # если событие - нажатие кнопки "Закрыть" (закончить диалог с Ботом)
                ...
                bot.write_msg(active_user, 'Для начала работы Бота, обратитесь к нему: "О Великий разум!"', 'edit',
                              False, msg_id)
